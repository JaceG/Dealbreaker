...
import * as imageUtils from '../../utils/functions/image'
...
const ReasonInputModal = ({
  ...
  const handleSubmit = async () => {
    try {
      ...
      for (let i = 0; i < attachments.length; i++) {
        ...
        console.log(
          `SUBMIT: Processing attachment ${i}, type: ${attachment.type}, URL: ${attachment.url.substring(0, 50)}...`
        )

        if (attachment.type === 'image') {
          console.log(`SUBMIT: Uploading image attachment ${i} to S3...`)
          try {
            // Generate a unique filename
            const timestamp = Date.now()
            const filename = `attachment_${timestamp}_${i}${attachment.url.endsWith('.png') ? '.png' : '.jpg'}`

            // Log details about the upload attempt
            console.log(
              `SUBMIT: Starting S3 upload for ${filename} from ${attachment.url.substring(0, 50)}...`
            )

            // Upload the image to S3
            console.log('SUBMIT: Calling uploadToS3 function')
            const s3Url = await uploadToS3(attachment.url, filename)
            console.log('SUBMIT: uploadToS3 returned:', s3Url)

            if (s3Url) {
              // Add the processed attachment with S3 URL
              console.log(`SUBMIT: S3 upload successful, URL: ${s3Url}`)
            ...
            processedAttachments.push({
              ...attachment,
              errorInfo: `S3 upload error: ${error.message || 'Unknown error'}`
            })
          }
        } else {
          // For non-image attachments, just pass them through
          console.log(`SUBMIT: Adding non-image attachment directly to list`)
  ...
  const pickMedia = async () => {
    ...
    if (IMAGE_PICKER_ENABLED && ImagePicker) {
      try {
        console.log('MEDIA PICKER: ImagePicker is enabled and available')
        // Request permission first
        const permissionResult =
          await ImagePicker.requestMediaLibraryPermissionsAsync()

        console.log('MEDIA PICKER: Permission result:', permissionResult)
        ...
        if (!result.canceled && result.assets && result.assets.length > 0) {
          ...
          for (const asset of result.assets) {
            console.log('MEDIA PICKER: Processing asset:', asset.uri)
            const type =
            ...
            newAttachments.push({
              type: type,
              url: asset.uri,
              name: `media-${Date.now()}-${newAttachments.length}.${type === 'video' ? 'mp4' : 'jpg'}`,
              timestamp: new Date().toISOString(),
          ...
          showToast(
            'success',
            `Added ${result.assets.length} ${result.assets.length === 1 ? 'item' : 'items'}`
          )
        } else {
          console.log('MEDIA PICKER: User cancelled or no assets selected')
        }
      } catch (error) {
      ...
      <View
        ...
        <View style={styles.modalContent}>
          <View style={styles.header}>
            <Text style={styles.title}>{modalTitle || 'Change Reason'}</Text>
          </View>
              ...
              {isPickerAvailable || __DEV__ ? (
                <>
                  <Text style={styles.attachmentHelperText}>
                    Add photos or videos to provide more context
                    {MOCK_AUDIO_ENABLED ? ', or voice memos' : ''}
                    {!IMAGE_PICKER_ENABLED && ' (Using Mock Data)'}
                  </Text>
                  ...
                  {MOCK_AUDIO_ENABLED ? (
                    <View style={styles.mediaButtonsRow}>
                      ...
                      <TouchableOpacity
                        style={[styles.mediaButton, styles.recordButton]}
                        onPress={addMockAudio}>
                        <Text style={styles.mediaButtonIcon}>🎙️</Text>
                        <Text style={styles.mediaButtonText}>VOICE MEMO</Text>
                      </TouchableOpacity>
                    </View>
                  ) : (
                  ...
                  {attachments.length > 0 ? (
                              ...
                              <View style={styles.audioControlsContainer}>
                                <TouchableOpacity
                                  ...
                                  onPress={() => {
                                    if (
                                      isPlaying &&
                                      playingAttachmentId === index
                                    ) {
                                      stopPlayback()
                                    } else {
                                      playAudio(index, attachment.url)
                                    }
                                  }}>
                                  <Text style={styles.audioPlayIcon}>
                                    {isPlaying && playingAttachmentId === index
                                      ? '⏸️'
                                      : '▶️'}
                                  </Text>
                                </TouchableOpacity>
                    ...
                    <View style={styles.noAttachmentsContainer}>
                      <Text style={styles.noAttachmentsText}>
                        ...
                        {MOCK_AUDIO_ENABLED ? 'buttons' : 'button'} above to add
                        {!IMAGE_PICKER_ENABLED ? ' sample' : ''} photos, videos
                        {MOCK_AUDIO_ENABLED ? ', or voice memos' : ''}.
                        {__DEV__ ? ' (Simulator Mode)' : ''}
                      </Text>
                    </View>
                  )}
                </>
                ...
                <View style={styles.pickerUnavailableContainer}>
                  <Text style={styles.pickerUnavailableText}>
                    Media attachments are unavailable. Please check app
                    permissions.
                  </Text>
                </View>
              )}
