...
export const uploadToS3 = async (fileUri, fileName) => {
  ...
  try {
    ...
    const backendUrl = Platform.select({
      ios: __DEV__
        ? 'http://localhost:4000/api/upload/direct'
        : 'https://dealbreaker-api.onrender.com/api/upload/direct',
      android: __DEV__
        ? 'http://10.0.2.2:4000/api/upload/direct'
        : 'https://dealbreaker-api.onrender.com/api/upload/direct'
    })
...
// Simple test function for S3 upload
export const testS3Upload = async () => {
  try {
    log('Starting S3 test upload...')

    // Create a test text file
    const fileName = `test_${Math.floor(Date.now() / 1000)}.txt`
    ...
    log(`Created test file at: ${filePath}`)
    ...
    if (url) {
      ...
      return {
        success: true,
        url,
        note: 'This is a simulated success - no actual upload occurred'
      }
    } else {
      log('Test upload failed')
      return {
        success: false,
