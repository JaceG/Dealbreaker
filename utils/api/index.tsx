...
export const getFlagHistory = async (profileId, flagId) => {
  try {
    ...
    const processedData = response.data.map(item => {
      ...
      if (item.attachments && Array.isArray(item.attachments)) {
        processedAttachments = item.attachments
            ...
            if (originalUrl) {
              ...
              if (
                ...
                if (originalUrl.startsWith('/')) {
                  ...
                  console.log(`Fixed file URL: ${originalUrl} → ${fixedUrl}`)
                }
              }
            }
          ...
          .filter(att => att !== null && att.url) // Remove nulls and attachments without URLs
      }
    ...
    console.log('History fetched successfully:', processedData.length, 'items')
        ...
        console.log(
          ...
          JSON.stringify(
            item.attachments.map(a => ({
              type: a.type,
              url: a.url ? a.url.substring(0, 30) + '...' : 'Missing URL'
            }))
          )
        )
      }
...
export const syncPendingChanges = async () => {
  try {
    ...
    if (isOfflineMode === 'true') {
      ...
      return { success: true, message: 'App is in offline mode, sync skipped.' }
    }
    ...
    try {
      ...
      console.error('API sync error:', apiError.message)
...
export const addAttachmentToHistory = async (historyId, attachment) => {
  try {
    ...
    if (attachment.isS3) {
      console.log('Attachment already has S3 URL, adding directly', attachment)
      ...
      try {
        ...
        if (apiError.response && apiError.response.status === 404) {
          console.log('History entry not found, storing attachment locally')
